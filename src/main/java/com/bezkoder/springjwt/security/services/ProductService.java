...
return ResponseEntity.ok(new ProductResponse(
  ...
  public ResponseEntity<?> findByNameContaining(ProductRequest productRequest) {
    ...
    if (paging==null)
     return ResponseEntity.ok(new ProductResponse(
      productRepository.findByNameContaining(productRequest.getName())
     ));
    else
    return ResponseEntity.ok(new ProductResponse(
      productRepository.findByNameContaining(productRequest.getName(),paging)
    ));
    }
 ...
 public ResponseEntity<?> findByNameContainingAndCategory(ProductRequest productRequest) {
  Pageable paging=checkPaging(productRequest);
    if (paging==null)
     return ResponseEntity.ok(new ProductResponse(
       productRepository.findByNameContainingAndCategory(productRequest.getName(),
     productRequest.getCategory())));
    else
    return ResponseEntity.ok(new ProductResponse(
      productRepository.findByNameContainingAndCategory(productRequest.getName(),
    productRequest.getCategory(),paging)));
    }
  ...
  public ResponseEntity<?> addToCart(CartRequest cartRequest) {
    ...
    return ResponseEntity.badRequest().body(new MessageResponse("Error: quantity must not bigger than 1"));
